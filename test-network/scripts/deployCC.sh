...
if [ "$CC_SRC_LANGUAGE" = "go" -o "$CC_SRC_LANGUAGE" = "golang" ] ; then
	...
	CC_SRC_PATH="../chaincode/test/go/"

	echo Vendoring Go dependencies ...
	pushd ../chaincode/test/go
	...
	CC_SRC_PATH="../chaincode/test/javascript/"

elif [ "$CC_SRC_LANGUAGE" = "java" ]; then
	CC_RUNTIME_LANGUAGE=java
	CC_SRC_PATH="../chaincode/test/java/build/install/test"

	echo Compiling Java code ...
	pushd ../chaincode/test/java
	./gradlew installDist
	popd
	...
	CC_RUNTIME_LANGUAGE=node # chaincode runtime language is node.js
	CC_SRC_PATH="../chaincode/test/typescript/"

	echo Compiling TypeScript code into JavaScript ...
	pushd ../chaincode/test/typescript
	npm install
	npm run build
	popd
...
packageChaincode() {
  ...
  setGlobals $PEER $ORG
  set -x
  peer lifecycle chaincode package test.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label test_${VERSION} >&log.txt
  res=$?
  set +x
  cat log.txt
...
installChaincode() {
  ...
  setGlobals $PEER $ORG
  set -x
  peer lifecycle chaincode install test.tar.gz >&log.txt
  res=$?
  set +x
  cat log.txt
...
queryInstalled() {
  ...
  set +x
  cat log.txt
	PACKAGE_ID=$(sed -n "/test_${VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}" log.txt)
...
approveForMyOrg() {
  ...
  setGlobals $PEER $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name test --version ${VERSION} --init-required --package-id ${PACKAGE_ID} --sequence ${VERSION} >&log.txt
  set +x
  cat log.txt
...
checkCommitReadiness() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name test --version ${VERSION} --sequence ${VERSION} --output json --init-required >&log.txt
    res=$?
    set +x
    let rc=0
    for var in "$@"
...
commitChaincodeDefinition() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --channelID $CHANNEL_NAME --name test $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} --init-required >&log.txt
    res=$?
    set +x
  else
    set -x
    peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name test $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} --init-required >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
queryCommitted() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name test >&log.txt
    res=$?
    set +x
...
chaincodeInvokeInit() {
  ...
  # it using the "-o" option
  set -x
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n test $PEER_CONN_PARMS --isInit -c '{"function":"init","Args":[]}' >&log.txt
  res=$?
  set +x
  cat log.txt
...
#     set -x
#     peer chaincode query -C $CHANNEL_NAME -n test -c '{"Args":["queryAllCars"]}' >&log.txt
#     res=$?
#     set +x
# 		let rc=$res
# 		COUNTER=$(expr $COUNTER + 1)
...
# # chaincodeQuery 1
echo
echo "========= All GOOD, JEA Digital Business Network execution completed =========== "
echo
exit 0